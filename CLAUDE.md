...
- **Games tab:** kid `/dashboard/games` hub (`nav.games`, 🎮 sidebar entry, `games` `bgVariant`) with three frontend-only games under `pages/dashboard/games/` — no backend state, no star payouts; best scores persist per user per game in localStorage (`bestScore.ts`). **Memory Match** (`/memory`): DOM/CSS flip cards (padding-bottom square trick — old tablets lack `aspect-ratio`), emoji pairs from `memoryDeck.ts` (pure, rng-injectable), easy/medium/hard (6/8/10 pairs), best = fewest moves. **Simon** (`/simon`): 2×2 color pads, `playSimonPad` C-major tones in `lib/sound.ts`, timeout-chained playback that speeds up per round, best = rounds completed. **Star Catcher** (`/catcher`): Canvas 2D + rAF with pure simulation in `catcherEngine.ts` (spawn/fall/catch/miss, ramping speed — unit-tested without DOM), pointer-driven basket (`touch-action: none`), 3 lives, emoji sprites via `fillText`. All three reuse `notifyCelebration`/`playReward`/`playWrong`, translations in `translations.py`, and follow the old-tablet CSS rules (no `aspect-ratio`/`inset`/flex-gap, transform+opacity transitions only). table pagination (reusable `Pagination`/`usePagination` in `components/Pagination.tsx` — client-side for chores/rewards/users/fulfillment/approvals, server-side via limit/offset for activity). Removed the manual stars-adjustment button from the admin users page.
