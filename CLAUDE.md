...
**Health:** 169 Vitest tests + 105 backend pytest tests all pass; the production build (`npm run build`) and `docker build` both succeed. `mypy --strict` on the backend still reports ~50 pre-existing SQLAlchemy `Column[T]` vs `T` errors (not introduced here).
...
- **Frontend:** Vitest (v3.2.4) + jsdom + React Testing Library + MSW. Run `npm test` in `frontend/`. Tests live in `src/**/*.test.{ts,tsx}`. Test config in `vitest.config.ts`, setup in `tests/setup.ts`. 169 tests across 24 files, all passing.
...
- **Backend:** pytest + httpx for FastAPI test client. Run `pytest` in `backend/`. 105 tests, all passing. Tests use the file database with an autouse fixture that deletes all rows after each test.
