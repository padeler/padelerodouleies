...
**In production.** The initial build plan is fully delivered (Phases 1–5 — see [Build history](#build-history)) and the app has been live in production on the LAN-only Synology NAS since early June 2026. Two large post-launch features have since **merged to `main` and shipped** in the release line (current tag **v0.15.1**):
...
**Health:** 262 Vitest tests + 305 backend pytest tests all pass; the production build (`npm run build`) and `docker build` both succeed. `mypy --strict` on the backend still reports pre-existing SQLAlchemy `Column[T]` vs `T` errors (not introduced here; the new exercises modules add only the same documented `Column[T]` class).
...
- **Security headers:** a `main.py` middleware sets CSP / `X-Frame-Options` / `nosniff` / `Referrer-Policy` (+ HSTS in prod). `script-src` is `'self'` **plus a sha256 hash of each inline script in the built `index.html`**, computed at startup by `_index_script_hashes()` — `@vitejs/plugin-legacy` injects an inline SystemJS bootstrap that a bare `'self'` blocks, which leaves the whole SPA a blank page in production. Hashing the built file (rather than a hardcoded digest) survives Vite/plugin upgrades. User-content paths (`/avatars/`, `/chore-images/`, `/api/icons/svg/`) get a locked-down `default-src 'none'; sandbox` CSP; OpenAPI is hidden unless `EXPOSE_OPENAPI=1`.
...
- **Which engine to target:** the tablets are Samsung Tab 4 / Android 4.4, but they browse the app in **Opera (Chromium ~61)** — *not* the stock KitKat WebView (Chromium 30), which cannot run the SPA at all. `vite.config.ts` targets `chrome >= 61` for this reason. Flexbox, grid and `object-fit` all work there; `aspect-ratio` (Chromium 88) does not. Debug against Chromium 61 semantics — assuming the ancient WebView sends you chasing bugs that aren't there.
- **Percentage `padding-bottom` (the square/aspect-ratio trick) is not safe everywhere.** On that engine the padded box collapses to **zero height** when it is the *only* child of a column flexbox with indefinite height — the percentage resolves against the container's main size instead of its width. This silently emptied every image-only `multiple_choice`/`match_pairs` option button (`ordering` survived only because its items also carry a text sibling). For an image inside such a button, prefer an **in-flow `<img>` capped with `max-width`/`max-height`** (its height comes from the bitmap, so nothing can collapse, and it needs no `object-fit`) — see `.mc-option-img` / `.match-item-img` in `Exercises.css`. The trick remains fine inside a fixed-width box (`.order-item`) or a plain block (`.exercise-scene`).
...
- `frontend/public/exdiag.html` (+ `exdiag.js`) is an on-device CSS/image diagnostic served at **`/exdiag.html`**, for the LAN tablets where `chrome://inspect` is unavailable: it prints feature support, per-image HTTP status / decode result / painted box, and renders layout variants side by side so the device itself shows which one paints. ES5 with an external script (the prod CSP is `script-src 'self'`). Ships to production; delete both files once they are no longer useful.
...
- **Frontend:** Vitest (v3.2.4) + jsdom + React Testing Library + MSW. Run `npm test` in `frontend/`. Tests live in `src/**/*.test.{ts,tsx}`. Test config in `vitest.config.ts`, setup in `tests/setup.ts`. 262 tests across 31 files, all passing.
