...
Phases 1–4 complete. Phase 5 M5.1 (responsive polish & accessibility) complete. M5.6 (chore form rework + notification system) complete. Schema update: Chore/Reward models now use single `title`/`description` fields (removed `*_el`/`*_en` pairs), `Chore.start_time` is nullable, `User.username` is unique case-insensitive, `HistoryLedger` has `action_label` column, `User` has `preferred_theme` column (`system`/`light`/`dark`). Bugs fixed: missing setup icons, user self-deletion, chore 422 on missing start_time, image avatar double-prefix on login page, logout stale redirect, WebSocket double-accept error. Features added: theme system (user-controlled dark/light/system), keyboard PIN input, lazy icon loading, touch-friendly toggle-based chore form UI, toast notification system with confetti celebrations. Remaining: M5.2 (Dockerfile), M5.3 (docker-compose), M5.4 (LAN testing), M5.5 (handover docs). Frontend has 113 Vitest tests (all passing) + 60 backend pytest tests. When asked to implement features, treat `PLAN.md` as the authoritative roadmap and `README.md` as the spec; if either disagrees with code that gets written later, the code wins but flag the drift.
