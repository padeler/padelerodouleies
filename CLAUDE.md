...
- **Games tab:** a hub for **both roles** — kid `/dashboard/games` and admin `/admin/games` (`nav.games`, 🎮 sidebar entry, `games` `bgVariant`), reusing the same components under `pages/dashboard/games/` (the `GamePage` back link is base-relative — `to=".." relative="path"` — so it resolves under either mount). Three frontend-only games, no star payouts; **best scores are server-backed per user** via the `GameScore` table (`GET`/`POST /api/games/scores`, `app/services/games.py`, `useGameScores.ts`), so they roam across devices and feed the Stats games scoreboard. **Memory Match** (`/memory`): DOM/CSS flip cards (padding-bottom square trick — old tablets lack `aspect-ratio`), emoji pairs from `memoryDeck.ts` (pure, rng-injectable; pool restricted to Unicode ≤6.1 so every tile renders on the old tablets), easy/medium/hard (6/8/10 pairs), best = fewest moves. **Simon** (`/simon`): 2×2 color pads, `playSimonPad` C-major tones in `lib/sound.ts`, timeout-chained playback that speeds up per round, best = rounds completed. **Star Catcher** (`/catcher`): Canvas 2D + rAF with pure simulation in `catcherEngine.ts` (spawn/fall/catch/miss, ramping speed — unit-tested without DOM), pointer-driven basket (`touch-action: none`), 3 lives, `⭐`/`💩` item sprites via `fillText` and a rasterised Lucide `shopping-basket` SVG for the basket. All three reuse `notifyCelebration`/`playReward`/`playWrong`, translations in `translations.py`, and follow the old-tablet CSS rules (no `aspect-ratio`/`inset`/flex-gap, transform+opacity transitions only). table pagination (reusable `Pagination`/`usePagination` in `components/Pagination.tsx` — client-side for chores/rewards/users/fulfillment/approvals, server-side via limit/offset for activity). Removed the manual stars-adjustment button from the admin users page.
