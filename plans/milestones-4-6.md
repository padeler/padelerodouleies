...
- [ ] `BundlePlayer.tsx`: extend the type switch (currently `numeric_entry` /
...
- [ ] Confirm `lib/types.ts::ExerciseView` covers every field the players read
...
- [ ] Reset behaviour on a wrong answer: ordering/match must clear their in-progress
...
- [ ] Add player styles to `Exercises.css`: no `filter: blur`, transform/opacity
...
- [ ] Add any new player chrome strings to `backend/app/i18n/translations.py`
...
- [ ] Extend `backend/scripts/make_sample_bundles.py` to emit at least one bundle
...
- [ ] Each new sample must pass the M1 validator (`python -m app.schemas.exercises
      <dir>`) — the script already validates each bundle it writes; keep that.
- [ ] Regenerate `samples/exercises/` and update `samples/exercises/README.md` if
      the bundle inventory changes.

### Tests
- [ ] Vitest player tests under `pages/dashboard/exercises/`: counting taps the
...
- [ ] Backend: `grade()` for the three types is already covered — add a
...
### Manual rescan
- [ ] Backend `POST /api/admin/exercises/rescan` in `app/api/admin.py`: invalidate
...
- [ ] `app/services/exercise_bundles.py`: expose a cache-clear hook the endpoint
      calls (fail-explicit: log what was re-scanned and any new invalid dirs).
- [ ] Frontend: a "Rescan" button on `AdminExercisesPage.tsx` (Lucide icon, not
...
- [ ] Test: pytest for the endpoint (admin-only; returns counts); a Vitest click
...
- [ ] Add per-kid solved/earned exercise counts to the kid-facing Stats tab
...
- [ ] Gate behind the same `User.is_active` filter as the rest of Stats (invariant
      #8 — soft-deleted kids excluded). Add the `Stats*` TS type + `stats.*`
      translations for the new section.
- [ ] Tests: pytest for the stats payload field; Vitest for the rendered section.
...
### The skill
- [ ] `tools/exercise-gen/`: a Claude Code project skill (`SKILL.md` + prompt
      templates) whose contract is:
...
- [ ] Document the workflow in `tools/exercise-gen/README.md` (inputs, how to run,
